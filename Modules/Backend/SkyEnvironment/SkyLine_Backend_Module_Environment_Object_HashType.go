...
//															━━━━┛
//
// Filename          | SkyLine_Backend_Module_Environment_Object_HashType
// Extension         | .go ( golang source code file )
// Purpose           | Defines required states for the Hash data type
...
func (SL_ObjectHashMap *SL_HashMap) SkyLine_ObjectFunction_GetDataType() ObjectDataType {
	return SKYLINE_DATATYPE_HASH_OBJECT
}
...
func (SL_ObjectHashMap *SL_HashMap) Reset() {
	SL_ObjectHashMap.Offset = 0
}

func (SL_ObjectHashMap *SL_HashMap) SkyLine_Next_Itteration() (SL_Object, SL_Object, bool) {
	if SL_ObjectHashMap.Offset < len(SL_ObjectHashMap.Pairs) {
		idx := 0

		for _, pair := range SL_ObjectHashMap.Pairs {
			if SL_ObjectHashMap.Offset == idx {
				SL_ObjectHashMap.Offset++
...
func (SL_ObjectHashMap *SL_HashMap) SkyLine_ObjectFunction_GetTrueValue() string {
	...
	HPS := make([]string, 0)
	for _, pair := range SL_ObjectHashMap.Pairs {
		HPS = append(HPS, fmt.Sprintf("\n\t%s => %s->%s", pair.Key.SkyLine_ObjectFunction_GetTrueValue(), pair.Key.SkyLine_ObjectFunction_GetDataType(), pair.Value.SkyLine_ObjectFunction_GetTrueValue()))
	}
	Out.WriteString("map{")
	Out.WriteString(strings.Join(HPS, ", "))
	Out.WriteString("\n};")
	return Out.String()
}
...
func (SL_ObjectHashMap *SL_HashMap) SkyLine_ObjectFunction_InvokeObject(Call string, Environ SkyLineEnvironment, InvokeArgs ...SL_Object) SL_Object {
	if cx, ok := SL_InvokeHashMapMap[Call]; ok {
		return cx("", Environ, SL_ObjectHashMap, InvokeArgs...)
	} else {
