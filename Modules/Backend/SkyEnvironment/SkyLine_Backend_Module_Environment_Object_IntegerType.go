...
//															━━━━┛
//
// Filename          | SkyLine_Backend_Module_Environment_Object_IntegerType
// Extension         | .go ( golang source code file )
// Purpose           | Defines required states for the Integer data type
...
func (SL_ObjectInteger *SL_Integer) SkyLine_ObjectFunction_GetTrueValue() string {
	return fmt.Sprintf("%d", SL_ObjectInteger.Value)
}
...
func (SL_ObjectInteger *SL_Integer) SkyLine_ObjectFunction_InvokeObject(Call string, Environ SkyLineEnvironment, InvokeArgs ...SL_Object) SL_Object {
	if cx, ok := SL_InvokeIntegerMap[Call]; ok {
		return cx(SL_ObjectInteger.Value, Environ, InvokeArgs...)
	} else {
