...
//															━━━━┛
//
// Filename          | SkyLine_Backend_Module_Environment_Object_ArrayType
// Extension         | .go ( golang source code file )
// Purpose           | Defines required states for the Array data type
...
func (SL_ObjectArray *SL_Array) SkyLine_ObjectFunction_GetDataType() ObjectDataType {
	return SKYLINE_DATATYPE_ARRAY_OBJECT
}
...
func (SL_ObjectArray *SL_Array) SkyLine_ObjectFunction_GetTrueValue() string {
	...
	ObjectElements := make([]string, 0)
	for _, elem := range SL_ObjectArray.Elements {
		ObjectElements = append(ObjectElements, elem.SkyLine_ObjectFunction_GetTrueValue())
	}
	Out.WriteString("[")
	Out.WriteString(strings.Join(ObjectElements, ", "))
	Out.WriteString("];")
	return Out.String()
}
...
func (SL_ObjectArray *SL_Array) SkyLine_Next_Itteration() (SL_Object, SL_Object, bool) {
	if SL_ObjectArray.Offset < len(SL_ObjectArray.Elements) {
		SL_ObjectArray.Offset++
		element := SL_ObjectArray.Elements[SL_ObjectArray.Offset-1]
		return element, &SL_Integer{Value: SL_ObjectArray.Offset - 1}, true
	}
...
func (SL_ObjectArray *SL_Array) SkyLine_ObjectFunction_InvokeObject(Call string, Environ SkyLineEnvironment, InvokeArgs ...SL_Object) SL_Object {
	// return methods
	if cx, ok := SL_InvokeArrayMap[Call]; ok {
		return cx(SL_ObjectArray.Elements, SL_ObjectArray, Environ, InvokeArgs...)
	} else {
		return &SL_Error{Message: "Call did not exist"}
	}
}
