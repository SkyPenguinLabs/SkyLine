...
//
// File Contains -> This file contains any other sub workers within the language that may be able to work with everything currently in the works like the mid point function

package SkyLine_Configuration_Engine_Backend_Source

func SL_IDENT_LOOKUP(identifier string) TokenDataType {
	if token, ok := KeyMap[identifier]; ok {
		return token
	}
	return IDENTIFIER
}
