...
func Array_Includes(Values []SkyEnv.SL_Object, ArrayObject *SkyEnv.SL_Array, Env SkyEnv.SkyLineEnvironment, args ...SkyEnv.SL_Object) SkyEnv.SL_Object {
	if x := SkyLine_Standard_Library_Helper_CheckArguments(
		...
		valuetocheck := args[0].(*SkyEnv.SL_String).Value
		for i := 0; i < len(ArrayObject.Elements); i++ {
			if secval, ok := ArrayObject.Elements[i].(*SkyEnv.SL_String); ok {
				if strings.Compare(valuetocheck, secval.Value) == 0 {
