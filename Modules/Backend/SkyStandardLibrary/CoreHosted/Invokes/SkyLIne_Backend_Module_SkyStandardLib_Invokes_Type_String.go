...
func String_Pop(Value string, Env SkyEnv.SkyLineEnvironment, args ...SkyEnv.SL_Object) SkyEnv.SL_Object {
	if x := SkyLine_Standard_Library_Helper_CheckArguments(
		"String.Pop",
		args,
		SkyLine_Standard_Library_Helper_ArgumentCheckDataType(SkyEnv.SKYLINE_DATATYPE_STRING_OBJECT),
	); x != nil {
		return &SkyEnvironment.SL_Error{Message: x.Error()}
	}
	var values []string
	for i := 0; i < len(args); i++ {
		if arg, ok := args[i].(*SkyEnv.SL_String); ok {
			values = append(values, arg.Value)
		}
	}
	for _, val := range values {
		Value = strings.ReplaceAll(Value, val, "")
	}
	return &SkyEnvironment.SL_String{Value: Value}
}

func String_Contains(Value string, Env SkyEnv.SkyLineEnvironment, args ...SkyEnv.SL_Object) SkyEnv.SL_Object {
	if x := SkyLine_Standard_Library_Helper_CheckArguments(
		"String.Contains",
		args,
		SkyLine_Standard_Library_Helper_ArgumentCheckDataType(SkyEnv.SKYLINE_DATATYPE_STRING_OBJECT),
		SkyLine_Standard_Library_Helper_ExactArguments(1),
	); x != nil {
		return &SkyEnvironment.SL_Error{Message: x.Error()}
	} else {
		value := args[0].(*SkyEnv.SL_String).Value
		if strings.Contains(Value, value) {
			return &SkyEnvironment.SL_Boolean{Value: true}
		}
		return &SkyEnvironment.SL_Boolean{Value: false}
	}
}

func String_Index(Value string, Env SkyEnv.SkyLineEnvironment, args ...SkyEnv.SL_Object) SkyEnv.SL_Object {
	if x := SkyLine_Standard_Library_Helper_CheckArguments(
		"String.Index",
		args,
		SkyLine_Standard_Library_Helper_ArgumentCheckDataType(SkyEnv.SKYLINE_DATATYPE_INTEGER_OBJECT),
		SkyLine_Standard_Library_Helper_ExactArguments(1),
	); x != nil {
		return &SkyEnvironment.SL_Error{Message: x.Error()}
	} else {
		val := args[0].(*SkyEnv.SL_Integer).Value
		if len(Value) < val {
			return &SkyEnvironment.SL_NULL{}
		}
		return &SkyEnvironment.SL_String{Value: Value[0:val]}
	}
}
