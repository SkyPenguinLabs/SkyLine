...
func SkyLine_Standard_Library_Helper_GetMethods(DataType string, Env SkyEnvironment.SkyLineEnvironment) SkyEnvironment.SL_Object {
	Dynamic_Environment := Env.Names(DataType)
	var Callable []string
	Callable = append(Callable, Static_Integer_Methods...)
...
func CheckIntegerType(Obj SkyEnvironment.SL_Object) bool {
	switch Obj.(type) {
	case *SkyEnvironment.SL_Integer:
		return true
	case *SkyEnvironment.SL_Integer8:
		return true
	case *SkyEnvironment.SL_Integer16:
		return true
	case *SkyEnvironment.SL_Integer32:
		return true
	case *SkyEnvironment.SL_Integer64:
		return true
	default:
		return false
	}
