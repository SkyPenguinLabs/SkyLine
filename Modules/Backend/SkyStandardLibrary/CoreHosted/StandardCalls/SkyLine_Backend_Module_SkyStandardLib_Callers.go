...
import (
	...
	SkyHelpers "github.com/SkyPenguinLabs/SkyLine/Modules/Backend/SkyStandardLibrary/CoreHosted/GenuineCoreHelpers"
)

func UserRoot(args ...SkyEnv.SL_Object) SkyEnv.SL_Object {
	// No arguments, just return boolean
	u, x := user.Current()
	if x != nil {
		return &SkyEnv.SL_NULL{}
	}
	return &SkyEnv.SL_Boolean{Value: u.Uid == "0" || u.Gid == "0"}
	// if false, then the user is not root and if trye then the user is root
}
...
func Halt(args ...SkyEnv.SL_Object) SkyEnv.SL_Object {
	if x := SkyHelpers.SkyLine_Standard_Library_Helper_CheckArguments(
		"halt",
		args,
		SkyHelpers.SkyLine_Standard_Library_Helper_ArgumentCheckDataType(SkyEnv.SKYLINE_DATATYPE_INTEGER_OBJECT, SkyEnv.SKYLINE_DATATYPE_STRING_OBJECT),
		SkyHelpers.SkyLine_Standard_Library_Helper_ExactArguments(2),
	); x != nil {
		return &SkyEnv.SL_Error{Message: x.Error()}
	}
	count := args[0].(*SkyEnv.SL_Integer).Value
	timetype := args[1].(*SkyEnv.SL_String).Value
	...
	time.Sleep(time.Duration(count) * tt)
	return &SkyEnv.SL_NULL{}
}

func Quit(args ...SkyEnv.SL_Object) SkyEnv.SL_Object {
	if x := SkyHelpers.SkyLine_Standard_Library_Helper_CheckArguments(
		"quit",
		args,
		SkyHelpers.SkyLine_Standard_Library_Helper_ArgumentCheckDataType(SkyEnv.SKYLINE_DATATYPE_INTEGER_OBJECT),
		SkyHelpers.SkyLine_Standard_Library_Helper_ExactArguments(1),
	); x != nil {
		return &SkyEnv.SL_Error{Message: x.Error()}
	}
	code := args[0].(*SkyEnv.SL_Integer).Value
	os.Exit(code)
	return &SkyEnv.SL_NULL{}
}
